...
public_inventory = Blueprint('public_inventory', __name__)
...
@public_inventory.before_request
def adjust_locale_inv():
    m.set_lang(str(flask_babel.get_locale()))


@public_inventory.route('/')
def index():
    return render_template('public_inventory/index.html', manager=m, MAX_APIKEY=MAX_APIKEY)


@public_inventory.route('/error/<error_type>')
def error(error_type):
    if error_type == 'invalid_key':
        return render_template('public_inventory/error_invalid_key.html')
    if error_type == 'invalid_permission':
        return render_template('public_inventory/error_invalid_permission.html')


@public_inventory.route('/list/', methods=['POST'])
def list():
    guilds_id = []
    ...
    return render_template('public_inventory/list.html', manager=m)
